	...
	function gLoop(i) {
		if(i < results.length) {
			const PS_API = {
					...
					results[i] + `&key=AIzaSyAaBLhs1pKLTQyGD8CT0N4CSeqM2q-6G7s`};
			request(PS_API, function(error, response, html) {
				if (!error) {
					const json = JSON.parse(html);
					console.log(json);
					console.log(results[i]);
					if(json.ruleGroups !== undefined) {
						if(json.ruleGroups.SPEED !== undefined) {
							console.log('Speed: ' + json.ruleGroups.SPEED.score);
						}
						if(json.ruleGroups.USABILITY !== undefined) {
							console.log('Usability: ' + json.ruleGroups.USABILITY.score);
						}
					}
					console.log("");
					i++;
